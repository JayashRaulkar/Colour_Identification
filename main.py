import pandas as pd
import cv2
global xpos, ypos, r , b , g, clicked
clicked=False
img_path=input("enter img path:  ")
#img_path = "Color detection/pic1.jpg"
csv_path = "Color detection/colors.csv"
...
while(True):
    ...
    if clicked:
        ...
        text= get_color(r,b,g)
        cv2.putText(img,text,org=(23,51),fontFace=2,fontScale=0.8,color=(0,0,0),thickness=1,lineType=cv2.LINE_AA)
        if (r+b+g<600):
            cv2.putText(img,text,org=(23,51),fontFace=2,fontScale=0.8,color=(255,255,255),thickness=1,lineType=cv2.LINE_AA)
